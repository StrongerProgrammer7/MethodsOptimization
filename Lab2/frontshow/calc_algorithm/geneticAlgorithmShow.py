    ...
    try:
        if(len(gv.x_data) > 0 and len(gv.y_data) > 0 and isNotEmptyFields([tf_populationSize,tf_numGeneratics])):
            ...
            if(type(gv.START) == dict):
                ...
                best_solution, best_fitness,points = genetic_algorithm( min_x, max_x, min_y, max_y,population_size, num_generations,gv.current_function)
            else:
                best_solution, best_fitness, points = genetic_algorithm(gv.START, gv.END, gv.START, gv.END, population_size,
