...
def createTab_Genetic(tab,call_function):
    ...
    generatics_btn = otk.ttk.Button(tab, text='Вызвать генетический', command=lambda: call_function(textField_populationSize,textField_numGeneratics,lab_optimalFunc,lab_optimalValuePoints))
    generatics_btn.grid(row=6, column=1, columnspan=3, padx=5, pady=5, sticky="nsew")

    gv.textReachGeneraticPoints = otk.ScrolledText(tab, height=10, width=30)
    gv.textReachGeneraticPoints.grid(row=7, column=1, columnspan=3, padx=5, pady=5, sticky="nsew")
