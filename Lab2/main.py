...
def buildBaseFunction():
    ...
    ax_3d.plot_surface(X, Y, Z, cmap=ColorFigure.VIRIDIS.value,alpha=0.8, antialiased=True,rstride=1, cstride=1)
    canvas_3d.draw()
...
def createTab1(frame1):
    createLabel.placement_label(frame1, "Choose count iteration for gradient", 0, 1, 5, 1, 3, 5)
    textFieldCountIter = ttk.Entry(frame1)
    textFieldCountIter.insert(0, "500")

    findMinGradientDescent_button = ttk.Button(frame1, text="Call gradient Descent",
    ...
    textReachGradientPoint = ScrolledText(frame1,height=10,width=30)
    textReachGradientPoint.grid(row=3,column=1, columnspan=3, padx=5, pady=5, sticky="nsew")


def createTab2(frame2):
    findQud_btn = ttk.Button(frame2, text='Call simplex method', command=lambda: call_simplex_method())
    findQud_btn.grid(row=0, column=1, columnspan=3, padx=5, pady=5, sticky="nsew")
    global textReachQuadPoint
    textReachQuadPoint = ScrolledText(frame2, height=10, width=30)
...
if __name__ == '__main__':
    ...
    comboBoxFunc = ttk.Combobox(root, values=["function_1", "function_2", "Bila", "Buta", "Bukina", "Eggholder","quadratic"],
                                state="readonly")
